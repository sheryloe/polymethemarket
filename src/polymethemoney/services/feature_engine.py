...
class FeatureEngine:
    ...
    def _build_feature(self, tick: MarketTick) -> FeatureVector:
        ...
        trend_score = self._trend_score(window)
    ...
    @staticmethod
    def _realized_volatility(window: deque[float]) -> float:
        if len(window) < 5:
            return 0.0
        returns: list[float] = []
        for i in range(1, len(window)):
            prev = max(window[i - 1], 1e-6)
            curr = max(window[i], 1e-6)
    ...
    @staticmethod
    def _trend_score(window: deque[float]) -> float:
        if len(window) < 10:
            return 0.0
        first = max(window[0], 1e-6)
        last = max(window[-1], 1e-6)
